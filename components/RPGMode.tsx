...
import { RPGPlayerState, BattleQuestion, RPGEnemy, BattleQuestionType, RPGItem, RPGItemType, EnemyClass, TriviaCampaign, RPGItemQuality, ItemPerk, RPGPet } from '../types';
...
const CRIT_VFX_URL = "https://raw.githubusercontent.com/maysonw14-tech/asset/refs/heads/main/special%20effect/GIF.gif";
...
const ELEMENT_INFO = {
    fire: { 
        ...
        scaling: (lvl: number) => ({
            current: `150% Dmg. Burn ${20 + (lvl - 1) * 10}% of total atk/turn (Accumulative).`,
            next: `+10% Burning effect.`
        })
    },
    ice: { 
        name: 'Storm', 
    ...
    thunder: { 
        ...
        bg: 'bg-blue-900/40', 
        border: 'border-blue-500/30',
        desc: "Hits single target and splashes damage to others.",
        scaling: (lvl: number) => ({
            current: `Main: 100%. Splash: ${50 + (lvl - 1) * 10}%. Def Drop: ${10 + (lvl - 1) * 2}%.`,
...
const RPGMode: React.FC<RPGModeProps> = ({ onExit, initialLevel, onAddToLibrary, onOpenLibrary, globalStats }) => {
  ...
  // Ref to track latest enemies state for async callbacks (fixing stale closure issues)
  ...
  const handleSlotHover = (e: React.MouseEvent, item: RPGItem) => {
      setTooltipData({ 
          content: { type: 'item', data: item, equipped: player.equipment[item.type] }, 
          x: e.clientX, 
          y: e.clientY 
      });
  };
  ...
  // ... (rest of useEffects and logic remain similar)
  
  useEffect(() => {
    if (gamePhase === 'player_turn' && !turnProcessing && player.statusEffects.frozen > 0) {
        // Player is frozen, skip turn
  ...
  }, [gamePhase, turnProcessing, player.statusEffects.frozen]);
  ...
  const getItemEffectiveStats = (item: RPGItem) => {
    const qMult = 1 + (item.qualityBonus / 100);
    const b = item.baseStats;
    const scale: Record<RPGItemType, any> = { head: { mana: 0.01 }, chest: { def: 0.01 }, hands: { hp: 0.01 }, relic: { atk: 0.01 }, feet: { hp: 0.01 }, weapon: { atk: 0.015 } };
    const s = scale[item.type];
    return {
        atk: Math.floor(b.atk * qMult * (1 + (item.upgradeLevel * (s.atk || 0)))),
        def: Math.floor(b.def * qMult * (1 + (item.upgradeLevel * (s.def || 0)))),
        hp: Math.floor(b.hp * qMult * (1 + (item.upgradeLevel * (s.hp || 0)))),
        mana: Math.floor(b.mana * qMult * (1 + (item.upgradeLevel * (s.mana || 0))))
    };
  };
  ...
  const calculateStats = () => {
    ...
    (Object.values(player.equipment) as (RPGItem | null)[]).forEach(item => { 
        if (item) { 
            const s = getItemEffectiveStats(item); 
  ...
  // ... (Loot generation and other helper functions remain the same)
  const generateLoot = (): RPGItem => {
    ...
    if ((type === 'weapon' || type === 'relic') && selectedQual.q !== 'Common') {
        ...
        // 20% chance for elemental resistance perks on armor
        if (Math.random() < 0.2) {
            ...
            const val = Math.floor(Math.random() * 15) + 1; // 1-15%
  ...
  const startBattle = async (waveOverride?: number) => {
    ...
    setLobbyView('main'); // Reset lobby view to avoid stacking
  ...
  const initializeBattle = async (waveToPlay: number) => {
    // ... (Battle initialization remains same)
    ...
    try {
      ...
      // Capture base level from the first enemy to keep foes similar
      ...
      for (let i = 1; i < enemyCount; i++) {
          ...
          // Align level with baseLevel with slight variance (+/- 1)
          extraData.enemy.level = Math.max(1, baseLevel + Math.floor(Math.random() * 3) - 1);
          // Recalculate stats based on normalized level
      ...
      // COMP DIFFICULTY SCALING
      const growthFactor = 1.2; // 20% compounding
      ...
      battleEnemies.forEach(e => {
          ...
          e.level = waveToPlay; // Sync level with wave for visual clarity
          
          // Base stats logic (100 HP, 10 Atk, 5 Def) scaled
  ...
  const handleAnswerCheck = (accuracy: number) => {
    // ... (Answer check logic remains same)
    setTurnProcessing(true); 
    if (accuracy >= 60) { 
        ...
        if (question && question.correctAnswer) onAddToLibrary(question.correctAnswer, question.type); 
  ...
  const handleEnemyTurn = async () => {
      ...
      for (const enemy of livingEnemies) {
          ...
          if (activePet && activePet.hp > 0 && Math.random() < 0.4) {
              target = 'pet';
          }
          ...
          if (skillToUse) {
              ...
              if (skillToUse === 'light') {
                  ...
                  if (target === 'player') {
                       ...
                       if (player.hp - damage <= 0) playerDied = true;
                  } else {
                       setPlayer(prev => ({
                           ...prev,
      ...
      if (player.hp <= 0 || playerDied) { 
          ...
          setPlayer(prev => ({...prev, mana: Math.min(prev.maxMana, prev.mana + 5)}));
          if (activePet && activePet.hp > 0) {
               setPlayer(prev => ({...prev, pets: prev.pets.map(p => p.id === activePet.id ? {...p, mana: Math.min(p.maxMana, p.mana + 3)} : p)}));
          }
  ...
  // ... (Pet and player action logic remains the same)
  const executePetAction = async (skillKey: keyof typeof ELEMENT_INFO | 'normal', targetId: string | 'all' | 'player' | 'pet' | null) => {
      ...
      } else if (!targetId && skillKey !== 'light') { // Light can target ally
          const t = enemies.find(e => e.currentHp > 0);
          if (t) targetEnemies = [t];
      }

      // Handle Healing (Light)
      if (skillKey === 'light') {
          setIsCasting(true);
      ...
      setEnemies(prev => {
          ...
          const applyDamage = (e: RPGEnemy, dmg: number) => {
              ...
              let isCrit = Math.random() < (stats.critChance / 100);
  ...
  const executePlayerAction = async (skillKey: keyof typeof ELEMENT_INFO | 'normal' | 'capture', targetId: string | 'all' | 'player' | 'pet' | null) => {
                    ...
                    pets: prev.pets.map(p => {
                        if(p.id === activePet.id) {
                            return {
                                ...
                                statusEffects: {
                                    ...p.statusEffects,
                                    frozen: Math.max(0, (p.statusEffects?.frozen || 0) - 1)
                                }
                            }
                        }
                        return p;
    ...
    if (skillKey === 'capture') {
        ...
        setIsCasting(true);
        setActiveVfx({ 
            url: "https://raw.githubusercontent.com/maysonw14-tech/asset/refs/heads/main/special%20effect/GIF.gif", 
            type: 'capture', 
            targetId: target.id 
        });
        ...
        if (Math.random() < successChance) {
            ...
            const newPet: RPGPet = {
                ...
                level: 0, 
                xp: 0, maxXp: 100, 
                hp: target.maxHp, maxHp: target.maxHp,
                mana: target.maxMana, maxMana: target.maxMana,
            ...
            // Allow floater to be seen before removing enemy
            await new Promise(r => setTimeout(r, 1200));

            setEnemies(prev => prev.map(e => e.id === target.id ? { ...e, currentHp: 0 } : e));
            
            // Explicitly check for win condition to avoid stale state
    ...
    setEnemies(prev => {
        ...
        const applyDamage = (e: RPGEnemy, dmg: number) => {
            if (e.currentHp <= 0) return;
            const evasion = (e.class === 'ghost' ? 0.15 : 0) + (e.class === 'rogue' ? 0.10 : 0);
            if (Math.random() < evasion && skillKey !== 'thunder' && skillKey !== 'ice') { 
    ...
    setTimeout(() => { 
        ...
        if (!enemyStillAlive) {
            handleVictoryTrigger();
        } else if (activePet && activePet.hp > 0) {
            if (activePet.statusEffects?.frozen > 0) {
                 setPlayer(prev => ({
                     ...prev,
                     pets: prev.pets.map(p => {
  ...
  const handleVictoryTrigger = () => {
      ...
      // Guaranteed loot for every victory, cranked up x10
      for (let i = 0; i < 10; i++) {
        lootItems.push(generateLoot());
      }

      // Calculate Pet XP Result OUTSIDE setPlayer to prevent closure issues
  ...
  const handleDefeatTrigger = () => {
      // ... (Defeat logic)
      setGamePhase('victory'); // Using victory phase UI logic but with defeat data
      ...
      // Calculate Pet XP Result OUTSIDE setPlayer
      let petResultData = null;
      const currentActivePet = player.pets.find(p => p.id === player.activePetId);
      ...
      setPlayer(prev => {
          ...
          return {
              ...
              coins: prev.coins + coins,
              pets: newPets,
              // No campaign progress on defeat
              hp: 0 // Ensure HP shows 0 until exit
          };
      });
  ...
  const handleVictory = (isCorrect: boolean) => {
      if (isCorrect) {
          handleVictoryTrigger();
      } else {
          handleDefeatTrigger();
  ...
  const handleBossTrivia = (correct: boolean) => {
      if (correct) {
          setVictoryData(prev => prev ? { ...prev, coins: prev.coins + 500, xp: prev.xp + 200 } : null);
      }
      setGamePhase('victory');
  };
  ...
  const equipItem = (item: RPGItem) => {
      ...
      // Clear selections instead of closing modal
  ...
  const showDamageFloat = (id: string, text: string, isCrit: boolean = false) => {
      ...
      setDamageFloaters(prev => [...prev, {
          ...
          color: isCrit ? 'text-yellow-400 drop-shadow-[0_0_10px_rgba(255,0,0,0.8)]' : (text.includes('HEAL') ? 'text-green-400' : (text.includes('FROZEN') ? 'text-blue-400' : 'text-white')),
          isCrit
      }]);
      setTimeout(() => {
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-black text-white flex flex-col font-mono animate-fade-in overflow-hidden">
      {/* ... (Styles and Effects remain the same) */}
      <style>{`
        ...
        .hero-attack { animation: hero-attack-frames 1s steps(6) forwards; }
        .hero-walk { animation: hero-walk-frames 1s steps(6) infinite; }
      ...
      {/* ... (Top Ribbon and other UI components remain the same) */}
      {gamePhase !== 'lobby' && (
      ...
      {/* LOBBY TOP RIBBON */}
      {gamePhase === 'lobby' && lobbyView !== 'blacksmith' && lobbyView !== 'map' && lobbyView !== 'character' && (
           <div className="absolute top-0 left-0 right-0 p-4 z-50 flex justify-between items-start pointer-events-none">
              <div className="flex items-center gap-3 pointer-events-auto">
                  {lobbyView !== 'main' && lobbyView !== 'pets' && lobbyView !== 'elements' && (
               ...
               <div className="absolute inset-0 z-[10] pointer-events-none">
                   ...
                   <div className="absolute top-[15%] right-[5%] w-[40%] h-[60%] grid grid-cols-2 gap-x-12 gap-y-4 items-center justify-items-center z-[20] pointer-events-none">
                       {enemies.map((enemy, index) => enemy.currentHp > 0 && (
                           <div 
                             ...
                             className={`relative flex flex-col items-center transition-all pointer-events-auto cursor-pointer group/enemy
                               ${selectingTarget ? 'hover:scale-110 hover:brightness-125 z-50' : ''}
                               ...
                               <div className={`w-24 h-24 relative bg-transparent ${actingEntity === enemy.id && enemyActionType === 'attack' ? 'animate-lunge-left' : (enemy.statusEffects.frozen > 0 ? '' : 'animate-float')}`}>
                                 ...
                                 {critTarget === enemy.id && (
                                     <div className="absolute inset-0 z-[90] pointer-events-none scale-150 flex items-center justify-center">
                                         <img src={CRIT_VFX_URL} className="w-full h-full object-contain mix-blend-screen" />
                                     </div>
                                 )}
                   ...
                   <div className="absolute top-[15%] left-[5%] flex flex-col items-start z-[20]">
                        <div className="flex items-end gap-12 relative pointer-events-auto ml-2 mt-8">
                            <div onClick={() => handleAllyClick('player')} className={`w-32 h-32 bg-transparent ${playerHurt ? 'brightness-75' : ''} transition-transform duration-[1800ms] ease-in-out relative ${selectingTarget && selectedSkill === 'light' ? 'cursor-pointer hover:scale-110 hover:brightness-125 z-50' : ''}`} style={{ transform: isApproaching ? 'translateX(50vw)' : 'translateX(0)' }}>
                            ...
                            {activePet && (
                                <div onClick={() => handleAllyClick('pet')} className={`w-24 h-24 relative mb-2 transition-all duration-[1000ms] ease-in-out ${activePet.hp <= 0 ? 'opacity-0' : 'opacity-90'} ${selectingTarget && selectedSkill === 'light' ? 'cursor-pointer hover:scale-110 hover:brightness-125 z-50' : ''}`} style={{ transform: petApproaching ? 'translateX(50vw)' : 'translateX(0)' }}>
                          ...
                          <div className="grid grid-cols-1 gap-3">
                              {player.pets.length === 0 ? (
                                  ...
                                  <div key={pet.id} onClick={() => { setSelectedPetId(pet.id); setDismissingPetId(null); }} className={`p-4 rounded-3xl border-2 transition-all cursor-pointer flex items-center gap-4 group hover:scale-[1.02] ${selectedPetId === pet.id ? 'bg-indigo-600/20 border-indigo-400 shadow-[0_0_30px_rgba(99,102,241,0.2)]' : 'bg-slate-900 border-white/5 hover:border-white/20'}`}>
                                      ...
                                      {player.activePetId === pet.id ? <div className="bg-emerald-500 text-black font-black text-[8px] px-2 py-1 rounded-full uppercase italic">Active</div> : <button onClick={(e) => { e.stopPropagation(); setPlayer(p => ({ ...p, activePetId: pet.id })); }} className="opacity-0 group-hover:opacity-100 px-3 py-1.5 bg-white/10 hover:bg-white/20 text-[9px] font-bold uppercase rounded-lg transition-all">Attend</button>}
                                  </div>
                              ))}
                          </div>
                      </div>
                              ...
                              <div className="w-full animate-pop-in space-y-6 flex-1 flex flex-col">
                                  ...
                                  <div className="flex flex-row gap-6 items-start">
                                       ...
                                       <div className="flex-1 flex flex-col justify-center space-y-3">
                                            ...
                                            {/* Growth Factor Badge */}
                                            <div className="has-tooltip relative inline-flex self-start cursor-help group/growth">
                                  ...
                                  <div className="grid grid-cols-3 gap-2">
                                      {[{ key: 'atk', label: 'Attack', icon: Sword, color: 'text-rose-400' }, { key: 'def', label: 'Defense', icon: Shield, color: 'text-amber-400' }, { key: 'maxHp', label: 'Health', icon: Heart, color: 'text-emerald-400' }, { key: 'maxMana', label: 'Mana', icon: Zap, color: 'text-blue-400' }, { key: 'critChance', label: 'Crit %', icon: TargetIcon, color: 'text-purple-400' }].map(s => (
                                          <div key={s.key} className="bg-black/60 p-2.5 rounded-xl border border-white/5 flex flex-col justify-center relative overflow-hidden group/stat">
                                      ...
                                      {/* Points Display */}
                                      <div className="bg-indigo-600/10 p-2.5 rounded-xl border border-indigo-500/30 flex flex-col justify-center items-center text-center">
                                  ...
                                  {/* Abilities */}
                                  <div className="bg-black/40 p-4 rounded-2xl border border-white/5 w-full text-left space-y-2">
                                      ...
                                      <div className="flex flex-wrap gap-2">
                                          {displayedStablesPet.skills.proactives.map(s => (
                                              <span key={s} className="px-2 py-1 bg-indigo-500/20 border border-indigo-500/40 rounded-lg text-indigo-300 text-[10px] font-bold uppercase">{s}</span>
                                          ))}
                                          {displayedStablesPet.skills.passives.map(s => (
                                              <span key={s} className="px-2 py-1 bg-emerald-500/20 border border-emerald-500/40 rounded-lg text-emerald-300 text-[10px] font-bold uppercase">{s}</span>
                                          ))}
                 ...
                 <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 pb-8">
                     ...
                     <div className="lg:col-span-4 bg-slate-900/40 border border-white/5 rounded-3xl p-4 flex flex-col min-h-[400px]">
                         ...
                         <div className="flex flex-wrap content-start gap-2 overflow-y-auto custom-scrollbar flex-1 pr-1 max-h-[500px]">
                             {player.inventory.map(item => {
                                 ...
                                 return (
                                    <button 
                                      ...
                                      onMouseEnter={(e) => handleSlotHover(e, item)}
                             ...
                             {/* Empty slots for visual grid feel */}
...
// ... (FixedTooltip and other components remain the same, just keeping them in file)
const FixedTooltip = ({ content, x, y }: { content: TooltipContent, x: number, y: number }) => {
    // Basic position adjustment to keep it on screen
    const style: React.CSSProperties = {
        top: y + 20,
        left: x - 100, // Center roughly on mouse x (reduced width offset)
        zIndex: 9999,
    };
...
const EnemyTooltip = ({ enemy }: { enemy: RPGEnemy }) => (
    <div className="flex flex-col gap-2">
         ...
         {(enemy.featureBonus || enemy.skills.passives.length > 0) && (
            <div className="p-1.5 bg-emerald-950/40 border border-emerald-500/20 rounded">
                <p className="text-[9px] text-emerald-400 font-bold uppercase mb-1 flex items-center gap-1"><ChevronsUp className="w-3 h-3"/> Buffs</p>
                <div className="flex flex-col gap-1">
                    {enemy.featureBonus && <span className="text-[9px] text-emerald-300 font-bold uppercase">{enemy.featureBonus}</span>}
                    {enemy.skills.passives.map(s => <span key={s} className="text-[9px] text-emerald-300 font-bold uppercase">{ELEMENT_INFO[s]?.name || s} (Lvl {enemy.skills.elementalLevel})</span>)}
                </div>
            </div>
         )}
         ...
         {(enemy.statusEffects.frozen > 0 || enemy.statusEffects.burning > 0 || enemy.statusEffects.defReductionTimer > 0) && (
          ...
          {enemy.skills.proactives.length > 0 && (
              <div className="border-t border-white/10 pt-2">
                  <p className="text-[9px] font-bold text-amber-300 uppercase mb-1 flex items-center gap-1"><Zap className="w-3 h-3"/> Active Skills</p>
                  <div className="flex flex-col gap-1">
                      {enemy.skills.proactives.map(s => (
                          <span key={s} className="text-[8px] text-indigo-200 uppercase font-bold">{ELEMENT_INFO[s]?.name || s} (Lvl {enemy.skills.elementalLevel})</span>
                      ))}
                  </div>
              </div>
          )}
...
const ItemTooltip = ({ item, currentEquipped }: { item: RPGItem, currentEquipped: RPGItem | null }) => {
    const qTheme = QUALITY_THEMES[item.quality]; const itemStats = { atk: item.baseStats.atk * (1 + item.qualityBonus/100), def: item.baseStats.def * (1 + item.qualityBonus/100), hp: item.baseStats.hp * (1 + item.qualityBonus/100), mana: item.baseStats.mana * (1 + item.qualityBonus/100) };
    const curStats = currentEquipped ? { atk: currentEquipped.baseStats.atk * (1 + currentEquipped.qualityBonus/100), def: currentEquipped.baseStats.def * (1 + currentEquipped.qualityBonus/100), hp: currentEquipped.baseStats.hp * (1 + currentEquipped.qualityBonus/100), mana: currentEquipped.baseStats.mana * (1 + currentEquipped.qualityBonus/100) } : { atk: 0, def: 0, hp: 0, mana: 0 };
    return (
        <div className="flex flex-col gap-2">
            ...
            <div className="flex flex-col gap-1">
                {Object.keys(itemStats).map(key => { const k = key as keyof typeof itemStats; if (itemStats[k] === 0 && curStats[k] === 0) return null; const diff = Math.floor(itemStats[k] - curStats[k]);
                    return (<div key={k} className="bg-black/30 p-1 rounded-lg border border-white/5 flex justify-between items-center"><span className="text-[8px] uppercase text-slate-500 font-bold">{k}</span><div className="flex items-center gap-1"><span className="text-[9px] font-black text-white">{Math.floor(itemStats[k])}</span>{diff !== 0 && (diff > 0 ? <TrendingUp className="w-2 h-2 text-emerald-400" /> : <TrendingDown className="w-2 h-2 text-rose-400" />)}</div></div>);
                })}
            </div>
        </div>
    );
...
const LootRevealCard: React.FC<{ item: RPGItem, onReveal: () => void }> = ({ item, onReveal }) => {
    ...
    return (
        <div 
            ...
            onMouseEnter={(e) => revealed && item && window.dispatchEvent(new CustomEvent('tooltip', { detail: { content: { type: 'item', data: item }, x: e.clientX, y: e.clientY } }))}
            onMouseLeave={() => revealed && window.dispatchEvent(new CustomEvent('tooltip', { detail: null }))}
        >
            {isRolling ? (
